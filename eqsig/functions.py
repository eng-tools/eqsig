...
def calc_step_fn_vals_error(values):
    """
    ...
    err_pre = np.sum(np.abs(pre_a - pre_mean[:, np.newaxis]), axis=1) - (npts - pre_n) * pre_mean
    err_post = np.sum(np.abs(post_a - post_mean[:, np.newaxis]), axis=1) - (npts - post_n) * post_mean
    ...
    err[-1] = np.sum(np.abs(values - np.mean(values)))
    return err
